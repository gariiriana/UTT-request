...
import { Mail, Lock, User, Eye, EyeOff, AlertCircle, Building2, Phone } from 'lucide-react';
import { DataCenterAnimation } from './DataCenterAnimation';

interface RegisterPageProps {
  onRegister: (name: string, email: string, password: string, role: string, siteProject: string, phoneNumber: string) => void;
...
export function RegisterPage({ onRegister, onSwitchToLogin, error }: RegisterPageProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!siteProject) {
      setValidationError('Site/Project is required');
      return;
    }

    onRegister(name, email, password, role, siteProject, phoneNumber);
  };
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-md">
          <div className="mb-8">
            ...
            <p className="text-gray-600">Register as a Project Manager</p>
          </div>
          ...
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg flex gap-3">
            ...
            <div>
              <p className="text-blue-900 mb-1">Registration Notice</p>
              <p className="text-blue-700 text-sm">
                Only <strong>Project Managers</strong> can register for an account. All other roles (PMO, Sales/Pre-Sales, Purchasing, BOD Finance, BOD Procurement, BOD Director) should log in using the credentials provided by PMO.
              </p>
            </div>
          </div>
          ...
          {(error || validationError) && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-600 mb-3">{error || validationError}</p>
              ...
              {error && (error.includes('sudah terdaftar') || error.includes('already')) && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-600 text-center">
                    Jika yakin email belum terdaftar, hubungi Admin di WhatsApp: 0857-2337-5324
                  </p>
                </div>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Role Display (Fixed as Project Manager) */}
            <div className="space-y-2">
            ...
            {/* Site/Project Input */}
            <div className="space-y-2">
              <label htmlFor="siteProject" className="block text-gray-700">
                Site/Project
              </label>
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
                ...
                <input
                  ...
                  type="text"
                  value={siteProject}
                  onChange={(e: ChangeEvent<HTMLInputElement>) => setSiteProject(e.target.value)}
                  placeholder="Enter your site/project"
                  required
                  className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                />
              </div>
